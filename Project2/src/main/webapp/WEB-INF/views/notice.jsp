<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="ko">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>왓챠</title>
  <link rel="stylesheet" href="<%=request.getContextPath() %>/resources/css/notice.css">
</head>
<body>
  <nav class="css-150hwvs">
    <ul class="css-5aqbxb">
      <li class="css-0"><a href="#">왓챠 익스클루시브</a></li>
      <li class="css-0"><a href="#">회사 정보</a></li>
      <li class="css-0"><a href="#">고객센터</a></li>
      <li class="css-0"><a href="#">채용정보</a></li>
      <li class="css-0"><a href="#">자주 묻는 질문</a></li>
    </ul><a class="css-mx6qp2" href="/"></a>
    <ul class="css-1qlincu">
      <li class="css-18zzine"><a class="css-1w7bkgk-StyledNavLink" href="/home">홈</a></li>
      <li class="css-18zzine">
        <div class="css-cssveg">
          <div class="css-0">
            <div class="css-s08p0c"><a class="css-1w7bkgk-StyledNavLink" href="/explore">탐색하기<span
                  class=" css-1sqt4fv">537</span></a></div>
          </div>
        </div>
      </li>
      <li class="css-18zzine"><a class="css-1w7bkgk-StyledNavLink" href="/evaluate">평가하기</a></li>
    </ul>
    <ul class="css-1wmmmu9">
      <li class="css-16x2in5">
        <div class="css-7zoqvg"><span class="css-1fcxk88"><span class="SVGInline css-q633x4">
              <svg class="SVGInline-svg css-q633x4-svg" width="16px" height="16px" viewBox="0 0 16 16" version="1.1"
                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                <title>Icons / GNB / Search / Inactive</title>
                <desc>Created with Sketch.</desc>
                <g id="Icons-/-GNB-/-Search-/-Inactive" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"
                  fill-opacity="0.88">
                  <g id="search" transform="translate(1.000000, 1.000000)" fill="#FFFFFF" fill-rule="nonzero">
                    <path
                      d="M13.9988333,12.8959444 L12.8959444,13.9988333 L8.72316667,9.82605556 C7.812,10.5081667 6.68461111,10.9172778 5.45883333,10.9172778 C2.44377778,10.9172778 0,8.4735 0,5.45883333 C0,2.44377778 2.44377778,0 5.45883333,0 C8.4735,0 10.9172778,2.44377778 10.9172778,5.45883333 C10.9172778,6.68461111 10.5081667,7.812 9.82605556,8.72316667 L13.9988333,12.8959444 L13.9988333,12.8959444 Z M5.45883333,1.55944444 C3.30516667,1.55944444 1.55944444,3.30516667 1.55944444,5.45883333 C1.55944444,7.61211111 3.30516667,9.35783333 5.45883333,9.35783333 C7.61211111,9.35783333 9.35783333,7.61211111 9.35783333,5.45883333 C9.35783333,3.30516667 7.61211111,1.55944444 5.45883333,1.55944444 L5.45883333,1.55944444 Z"
                      id="Shape"></path>
                  </g>
                </g>
              </svg></span>&nbsp;&nbsp;
            <!-- -->검색</span>
          <div class="css-s289sk">
            <div style="width:0;opacity:0.1" class="css-1rkym7a"></div>
          </div>
        </div>
      </li>
      <li class="css-16x2in5"><a class="css-1w7bkgk-StyledNavLink" href="/wishes">보고싶어요</a></li>
      <li class="css-16x2in5">
        <div class=" css-cssveg">
          <div class="css-0">
            <div class="css-s08p0c">
              <div class="css-1avnwm2">
                <div
                  src="https://dhgywazgeek0d.cloudfront.net/watcha/image/upload/c_fill,h_300,w_300/profile1_jomqrw.jpg"
                  class="css-3jhzp5"></div><span class="css-1w02mm1">최현준</span><span class="css-u703w"></span>
              </div>
            </div>
          </div>
        </div>
      </li>
    </ul>
  </nav>

  <section class="notice_area">
    <div class="notice">
      <h3>공지사항</h3>
      <details class="list">
        <summary>[왓챠] 데이터 초과 사용 민원 건에 관하여 사과의 말씀을 전합니다.</summary>
        <div class="tpt">
          안녕하세요. 왓챠입니다.
  
          최근 왓챠 어플리케이션으로 인해 사용자의 데이터가 과다하게 사용된 사례가 발생하여 왓챠를 이용하는 이용자분들께 걱정과 심려를 끼쳐드린 점에 대해 사과드립니다.
  
          한 이용자분의 제보를 통해 그동안 보고된 적 없었던 특수한 오류가 발생했음을 확인했습니다. 저희에게도 처음 보고된 오류라 이용자분의 제보 이후 원인을 파악하기까지 상당한 시간이 소요되었으며, 해당
          사실을 확인한 즉시 이용자분께 파악한 사실을 설명드리고 피해에 대한 보상 조치를 진행하였습니다. 그 과정에서 이용자분께 많은 심려와 불편함을 끼친 점에 대해 다시 한번 진심으로 사과드립니다.
  
          이번 사례와 관련하여, 많은 이용자분이 우려하시는 점에 대하여 왓챠 서비스의 기술적인 사양에 대한 설명을 드리고자 합니다. 왓챠의 서비스는 왓챠의 서버와 이용자 간의 통신을 통해 이루어지며, P2P
          트래픽 분산 로직 등 왓챠의 서버 트래픽 또는 타인의 트래픽을 다른 이용자에게 전가하는 형태는 왓챠 서비스 내 어떠한 곳에도 적용되어 있지 않습니다. 이용자분께서 모바일 기기를 통해 영상을 스트리밍으로
          감상하시는 경우, 영상 데이터를 작은 단위로 기기에 다운로드 받은 다음 이를 순서대로 정렬하여 화면에 재생하는 과정을 거칩니다. 현재까지 확인한 바로는 해당 모바일 기기에서 위와 같은 다운로드 요청이
          비정상적으로 많이 반복된 것이 원인이 된 것으로 보입니다.
  
          이 같은 오류가 발생한 더 정확한 원인이 무엇인지 파악하기 위한 심층적인 조사가 계속해서 진행 중이며, 현재까지 해당 오류 외에 추가 사례가 발견되지는 않았으나 정확한 현황 파악을 위해 모든 이용자를
          대상으로 전수 조사를 진행하고 있습니다.
  
          왓챠는 조사 결과가 나오는 즉시 문제의 원인 및 유사 사례의 존재, 유사한 피해를 겪은 이용자에 대한 보상 정책 및 재발방지 대책을 공유드릴 것을 약속합니다. 또한 이용자분들의 불편함에 대해 더욱
          빠르게 대응할 수 있도록 관련 프로세스도 개선할 것을 약속드립니다.
  
          피해를 입은 이용자 및 이번 사례로 인해 심려하시는 이용자들께 다시 한번 진심으로 사과드립니다.
  
          왓챠 드림
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.1</summary>
        <div class="tpt">
          안녕하세요. 왓챠입니다.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.2</summary>
        <div class="tpt">
          안녕하세요. 왓챠입니다.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.3</summary>
        <div class="tpt">
          안녕하세요. 왓챠입니다.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.4</summary>
        <div class="tpt">
          안녕하세요. 왓챠입니다.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
      <details class="list">
        <summary>공지사항 입니다.끝</summary>
        <div class="tpt">
          안녕하세요.
        </div>
      </details>
  
      <div class="btn">
        <button class="moreBtn">공지사항 더 불러오기</button>
      </div>
    </div>
    <script src="<%=request.getContextPath() %>/resources/js/notice.js"> </script>
  </section>

  <footer class="css-wvwa3p">
    <style data-emotion-css="1wn0ztv">
        .css-1wn0ztv {
            color: rgba(255, 255, 255, 0.2);
            font-size: 10px;
            font-weight: 400;
            -webkit-letter-spacing: -0.5px;
            -moz-letter-spacing: -0.5px;
            -ms-letter-spacing: -0.5px;
            letter-spacing: -0.5px;
        }
    </style>
    <ul class="css-1wn0ztv">
        <style data-emotion-css="1w1gghd">
            .css-1w1gghd {
                display: inline-block;
                position: relative;
                color: rgba(255, 255, 255, 0.7);
                font-size: 12px;
                font-weight: 400;
                -webkit-letter-spacing: -0.5px;
                -moz-letter-spacing: -0.5px;
                -ms-letter-spacing: -0.5px;
                letter-spacing: -0.5px;
                vertical-align: top;
                cursor: pointer;
            }

            .css-1w1gghd::after {
                content: "";
                display: inline-block;
                background: rgba(255, 255, 255, 0.2);
                vertical-align: top;
                width: 1px;
                height: 10px;
                margin: 5px 6px 0;
            }

            .css-1w1gghd:last-of-type::after {
                content: none;
            }
        </style>
        <li class="css-1w1gghd">왓챠피디아 서비스 이용약관</li>
        <style data-emotion-css="1czlaix">
            .css-1czlaix {
                display: inline-block;
                position: relative;
                color: #FFFFFF;
                font-size: 12px;
                font-weight: 700;
                -webkit-letter-spacing: -0.5px;
                -moz-letter-spacing: -0.5px;
                -ms-letter-spacing: -0.5px;
                letter-spacing: -0.5px;
                vertical-align: top;
                cursor: pointer;
            }

            .css-1czlaix::after {
                content: "";
                display: inline-block;
                background: rgba(255, 255, 255, 0.2);
                vertical-align: top;
                width: 1px;
                height: 10px;
                margin: 5px 6px 0;
            }

            .css-1czlaix:last-of-type::after {
                content: none;
            }
        </style>
        <li class="css-1czlaix">개인정보 처리 방침</li>
        <li class="css-1w1gghd">왓챠 서비스 이용약관</li>
        <li class="css-1w1gghd">
            <style data-emotion-css="1q0miqa">
                .css-1q0miqa {
                    color: rgba(255, 255, 255, 0.7) !important;
                    -webkit-text-decoration: none;
                    text-decoration: none;
                }
            </style><a href="https://watcha.zendesk.com/" target="_blank" rel="noopener noreferrer"
                class="css-1q0miqa">고객센터</a>
        </li>
        <li class="css-1w1gghd"><a href="https://team.watcha.com/" target="_blank" rel="noreferrer noopener"
                class="css-1q0miqa">채용정보</a></li>
    </ul>
    <style data-emotion-css="37p0v">
        .css-37p0v {
            -webkit-letter-spacing: -0.5px;
            -moz-letter-spacing: -0.5px;
            -ms-letter-spacing: -0.5px;
            letter-spacing: -0.5px;
            margin: 24px 0 0;
        }
    </style>
    <ul class="css-37p0v">
        <style data-emotion-css="thfupn">
            .css-thfupn {
                color: rgba(255, 255, 255, 0.7);
                font-size: 12px;
                line-height: 20px;
            }
        </style>
        <li class="css-thfupn">
            <style data-emotion-css="uw7vnf">
                .css-uw7vnf {
                    display: inline-block;
                    width: 139px;
                }
            </style><span class="css-uw7vnf">고객센터(이용 및 결제 문의)</span>
            <style data-emotion-css="92plnc">
                .css-92plnc::before {
                    content: "";
                    display: inline-block;
                    background: rgba(255, 255, 255, 0.2);
                    vertical-align: middle;
                    width: 1px;
                    height: 10px;
                    margin: 0 9px 0 0;
                }
            </style><span class="css-92plnc"><a href="mailto:cs@watcha.co.kr" target="_blank"
                    rel="noreferrer noopener" class="css-1q0miqa">cs@watcha.co.kr</a> <!-- -->/02-515-9985
                (유료)</span>
        </li>
        <li class="css-thfupn"><span class="css-uw7vnf">제휴 및 대외 협력</span><span class="css-92plnc"><a
                    href="https://watcha.team/contact" target="_blank" rel="noopener noreferrer"
                    class="css-1q0miqa">https://watcha.team/contact</a></span></li>
        <li class="css-thfupn"><span class="css-uw7vnf">B2B 이용권 구매 문의</span><span class="css-92plnc"><a
                    href="mailto:m_oper@coopnc.com" class="css-1q0miqa">쿠프마케팅 (ksm1459@coopnc.com /
                    070-4020-5289)</a></span></li>
    </ul>
    <style data-emotion-css="1quzgt9">
        .css-1quzgt9 {
            margin: 46px 0 0;
        }
    </style>
    <div class="css-1quzgt9">
        <style data-emotion-css="1b4iq4t">
            .css-1b4iq4t {
                color: rgba(255, 255, 255, 0.3);
                font-size: 12px;
                -webkit-letter-spacing: -0.5px;
                -moz-letter-spacing: -0.5px;
                -ms-letter-spacing: -0.5px;
                letter-spacing: -0.5px;
                vertical-align: middle;
                line-height: 20px;
            }
        </style>
        <ul class="css-1b4iq4t">
            <style data-emotion-css="1gvgzel">
                .css-1gvgzel {
                    display: inline-block;
                }

                .css-1gvgzel::after {
                    content: "";
                    display: inline-block;
                    background: rgba(255, 255, 255, 0.2);
                    vertical-align: top;
                    width: 1px;
                    height: 10px;
                    margin: 5px 6px 0;
                }

                .css-1gvgzel:last-of-type::after {
                    content: none;
                }
            </style>
            <li class="css-1gvgzel">주식회사 왓챠</li>
            <li class="css-1gvgzel">대표 박태훈</li>
            <li class="css-1gvgzel">서울특별시 서초구 강남대로 343 신덕빌딩 3층</li>
        </ul>
        <ul class="css-1b4iq4t">
            <li class="css-1gvgzel">사업자등록번호 211-88-66013</li>
            <li class="css-1gvgzel">통신판매업 신고번호 제 2019-서울서초-0965호</li>
        </ul>
    </div>
    <style data-emotion-css="x0g4fj">
        .css-x0g4fj {
            position: absolute;
            top: 181px;
            right: 50px;
        }

        @media (max-width:750px) {
            .css-x0g4fj {
                display: -webkit-box;
                display: -webkit-flex;
                display: -ms-flexbox;
                display: flex;
                position: initial;
                margin: 17px 0 11px;
            }
        }
    </style>
    <div class="css-x0g4fj">
        <style data-emotion-css="1epc1io">
            .css-1epc1io {
                display: inline-block;
                background: url(data:image/svg+xml;base64,) center no-repeat;
                background-size: 24px 24px;
                width: 36px;
                height: 36px;
                border: 1px solid #fff;
                border-radius: 50%;
                margin: 0 0 0 20px;
            }

            @media (max-width:750px) {
                .css-1epc1io:first-of-type {
                    margin-left: 0;
                }
            }
        </style><a
            src="data:image/svg+xml;base64,"
            href="https://fb.com/watchaKR" target="_blank" rel="noreferrer noopener"
            class="css-1epc1io"></a>
        <style data-emotion-css="v7oadi">
            .css-v7oadi {
                display: inline-block;
                background: url(data:image/svg+xml;base64,) center no-repeat;
                background-size: 24px 24px;
                width: 36px;
                height: 36px;
                border: 1px solid #fff;
                border-radius: 50%;
                margin: 0 0 0 20px;
            }

            @media (max-width:750px) {
                .css-v7oadi:first-of-type {
                    margin-left: 0;
                }
            }
        </style><a
            src="data:image/svg+xml;base64,"
            href="https://twitter.com/watcha_kr" target="_blank" rel="noreferrer noopener"
            class="css-v7oadi"></a>
        <style data-emotion-css="ix9zmt">
            .css-ix9zmt {
                display: inline-block;
                background: url(data:image/svg+xml;base64,) center no-repeat;
                background-size: 24px 24px;
                width: 36px;
                height: 36px;
                border: 1px solid #fff;
                border-radius: 50%;
                margin: 0 0 0 20px;
            }

            @media (max-width:750px) {
                .css-ix9zmt:first-of-type {
                    margin-left: 0;
                }
            }
        </style><a
            src="data:image/svg+xml;base64,"
            href="https://www.instagram.com/watcha_kr/" target="_blank" rel="noreferrer noopener"
            class="css-ix9zmt"></a>
        <style data-emotion-css="11668yy">
            .css-11668yy {
                display: inline-block;
                background: url(data:image/svg+xml;base64,) center no-repeat;
                background-size: 24px 24px;
                width: 36px;
                height: 36px;
                border: 1px solid #fff;
                border-radius: 50%;
                margin: 0 0 0 20px;
            }

            @media (max-width:750px) {
                .css-11668yy:first-of-type {
                    margin-left: 0;
                }
            }
        </style><a
            src="data:image/svg+xml;base64,"
            href="https://team.watcha.com/" target="_blank" rel="noreferrer noopener"
            class="css-11668yy"></a>
    </div>
    <style data-emotion-css="1ia10w4">
        .css-1ia10w4 {
            color: rgba(255, 255, 255, 0.3);
            font-family: "Helvetica Neue", Helvetica, serif;
            font-size: 12px;
            font-weight: 400;
            white-space: pre-line;
            margin: 3px 0 0;
        }

        .css-1ia10w4>em {
            font-weight: 700;
        }

        .css-1ia10w4>img {
            vertical-align: top;
            margin: 1px 0 0;
        }
    </style>
    <div class="css-1ia10w4"><a href="https://team.watcha.com/">
            <style data-emotion-css="175t0m4">
                .css-175t0m4 {
                    position: relative;
                    top: 1px;
                    margin-right: 3px;
                    margin-left: 1px;
                }
            </style><img
                src="data:image/svg+xml;base64,"
                width="42" alt="WATCHA" class="css-175t0m4" /></a> Copyright © 2021 by <em>Watcha, Inc.</em> All rights reserved.
      </div>
</footer>
</body>

</html>